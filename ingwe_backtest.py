...
# -------------------------------------------------------
BACKTEST_MODE            = True
BACKTEST_CSV            = "EURUSDc_M1_202601130552_202604200000.csv"
BACKTEST_SPEED          = 1
_backtest_index         = 0
_backtest_data          = None
...
def validate_candles(df: pd.DataFrame, session: str = None) -> bool:
    if df is None or len(df) < 50:
        ...
        return False
    if BACKTEST_MODE:
        # In backtest mode, we don't check for data freshness because we are using historical data
        return not has_frozen_prices(df)
    ...
    if BACKTEST_MODE:
        if _backtest_data is None:
            ...
            if not csv_path.exists():
                ...
                return None
            _backtest_data = pd.read_csv(csv_path, delimiter='\t')
            # Combine <DATE> and <TIME> columns into a single datetime column
            _backtest_data["time"] = pd.to_datetime(
                _backtest_data["<DATE>"] + " " + _backtest_data["<TIME>"],
                format="%Y.%m.%d %H:%M:%S",
                utc=True
            )
            # Rename OHLC columns to match expected format
            _backtest_data.rename(columns={
                "<OPEN>": "open",
                "<HIGH>": "high", 
                "<LOW>": "low",
                "<CLOSE>": "close"
            }, inplace=True)
            # Start with enough data for indicators to work (at least 100 candles)
            _backtest_index = max(0, min(100, len(_backtest_data) - 1))
            log(f"Backtest loaded: {len(_backtest_data)} candles from {BACKTEST_CSV}")
            log(f"Starting at index {_backtest_index} for sufficient indicator data")
