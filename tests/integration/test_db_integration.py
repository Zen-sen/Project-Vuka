...
def test_trades_table():
    ...
    if expected.issubset(columns):
        print(f"[OK] Trades table schema valid ({len(columns)} columns)")
    else:
...
def test_sessions_table():
    ...
    print(f"[OK] Sessions table: {session_count} records")
    ...
    if expected.issubset(columns):
        print(f"[OK] Sessions table schema valid ({len(columns)} columns)")
    else:
        print(f"[FAIL] Missing columns: {expected - columns}")
        return False
...
def test_loss_tracking_table():
    ...
    print(f"[OK] Loss tracking table: {loss_count} records")
    ...
    if expected.issubset(columns):
        print(f"[OK] Loss tracking schema valid ({len(columns)} columns)")
    else:
        print(f"[FAIL] Missing columns: {expected - columns}")
        return False
...
def test_sl_movements_table():
    ...
    print(f"[OK] SL movements table: {sl_count} records")
    ...
    if expected.issubset(columns):
        print(f"[OK] SL movements schema valid ({len(columns)} columns)")
    else:
        print(f"[FAIL] Missing columns: {expected - columns}")
        return False
...
def test_instance_locks_table():
    ...
    if expected.issubset(columns):
        print(f"[OK] Instance locks schema valid ({len(columns)} columns)")
    else:
        print(f"[FAIL] Missing columns: {expected - columns}")
        return False
...
def test_lock_acquire_release():
    ...
    if success:
        print(f"[OK] Lock acquired: {instance_tag}")
    else:
        print(f"[FAIL] Failed to acquire lock: {instance_tag}")
        return False
    ...
    if cursor.fetchone():
        print(f"[OK] Lock verified in database")
    else:
        print(f"[FAIL] Lock not found in database")
        return False
    ...
    if not cursor.fetchone():
        ...
        print(f"[FAIL] Lock still exists after release")
        return False
...
def test_insert_and_query():
    ...
    try:
        ...
        if result > 0:
            print(f"[OK] Test trade inserted (ID: {result})")
        else:
            print(f"[FAIL] Failed to insert test trade")
            return False
    except Exception as e:
        print(f"[FAIL] Error inserting trade: {e}")
        return False
    ...
    try:
        ...
        if len(trades) > 0:
            print(f"[OK] Query successful: {len(trades)} trades retrieved")
        else:
            print(f"[FAIL] No trades returned from query")
            return False
    except Exception as e:
        print(f"[FAIL] Error querying trades: {e}")
        return False
