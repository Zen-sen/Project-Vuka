...
def evaluate_ingwe(df, fvgs, sweep, sweep_level, price, atr, lot_size, session,
    ...
    if multiplier > 1.0:
        ...
        _log(f"London/NY Overlap -> Lot: {lot_size} (1.2x)")
    ...
    trend = get_h1_trend()
    if not trend:
        _log("H1 trend unclear.", "WARN")
        return
    _log(f"H1 Trend: {trend}")
    ...
    htf_bias_ok = True
    if not htf_bias:
        _log("HTF bias unavailable -- flagged for Kronos review.", "WARN")
        htf_bias_ok = False
    elif htf_bias != trend:
    ...
    if not level_sweep and asian_high and asian_low:
        if sweep == "SWEEP_HIGH" and abs(sweep_level - asian_high) < atr * 0.5:
            level_sweep = True
            _log(f"ASIAN HIGH SWEEP: {sweep_level:.5f} ~ AR {asian_high:.5f}  [+5]")
        elif sweep == "SWEEP_LOW" and abs(sweep_level - asian_low) < atr * 0.5:
            level_sweep = True
            _log(f"ASIAN LOW SWEEP: {sweep_level:.5f} ~ AR {asian_low:.5f}  [+5]")
    ...
    if m15_bos:
        _log(f"M15 BOS: {m15_bos}  (lookback={bos_lookback})")
        try:
        ...
        _log(f"M15 BOS: none confirmed  (lookback={bos_lookback})")
    ...
    for fvg_type, fvg_low, fvg_high, fvg_idx, ob, fvg_50 in fvgs:
        ...
        if sweep == "SWEEP_LOW" and fvg_type == "BULLISH_FVG" and trend == "BULLISH":
            ...
            if not dir_allowed:
                _log(f"Direction blocked: BUY ({dir_reason}).", "GUARD")
                continue
            if plus_di is None or minus_di is None or plus_di <= minus_di:
                _log(f"DI filter: +DI({plus_di}) <= -DI({minus_di}). Skip.", "GUARD")
                continue
            ...
            _log(f"Confluence [BUY REVERSAL]: {score}/120{bonus_label}")
            if score < threshold:
                _log(f"Score {score} < {threshold}. Flagging for Kronos review.", "WARN")
                score_ok = False
            else:
                score_ok = True
        ...
        if sweep == "SWEEP_HIGH" and fvg_type == "BEARISH_FVG" and trend == "BEARISH":
            if plus_di is None or minus_di is None or minus_di <= plus_di:
                _log(f"DI filter: -DI({minus_di}) <= +DI({plus_di}). Skip.", "GUARD")
                continue
            ...
            _log(f"Confluence [SELL REVERSAL]: {score}/120{bonus_label}")
            if score < threshold:
                _log(f"Score {score} < {threshold}. Waiting.", "GUARD")
                continue
            if not check_pre_trade_spread(atr):
                continue
            ctx = {
        ...
        if sweep == "SWEEP_LOW" and fvg_type == "BEARISH_FVG" and trend == "BEARISH":
            ...
            _log(f"Confluence [SELL CONTINUATION]: {score}/120{bonus_label}")
            if score < threshold:
                _log(f"Score {score} < {threshold}. Waiting.", "GUARD")
                continue
            if not check_pre_trade_spread(atr):
        ...
        if sweep == "SWEEP_HIGH" and fvg_type == "BULLISH_FVG" and trend == "BULLISH":
            ...
            if not htf_bias_ok:
                if session and "asian" in session.lower():
                    ...
                    _log(f"Asian split HTF -- raising Kronos threshold to {effective_buy_threshold}.", "GUARD")
                _log(f"HTF bias ({htf_bias}) not confirmed. Allowing Kronos to decide with BUY threshold {effective_buy_threshold}.", "WARN")
            if htf_bias != "BULLISH":
                _log(f"HTF bias ({htf_bias}) not bullish. Flagging for Kronos review.", "WARN")
            _log(f"Zone context: {_zone_context(df, price)}")
            bos_aligned = (m15_bos == "BULLISH_BOS")
            ...
            _log(f"Confluence [BUY CONTINUATION]: {score}/120{bonus_label}")
            if score < threshold:
                _log(f"Score {score} < {threshold}. Waiting.", "GUARD")
                continue
            if not check_pre_trade_spread(atr):
